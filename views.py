...
def top_page():
    ...
    s = "".join(
        [
            f"<a href={i}/overview>competiton: {i}</a><br>"
            for i in os.listdir("./competitions")
        ]
    )
    return s
...
def leaderboard_page(compe):
    ScoreCalculator = importlib.import_module(
        "competitions." + compe + ".ScoreCalculator"
    )
...
def submitresult(compe):
    ...
    try:
        ...
        return (
            "submited file failed to convert data frame. please check. <a href='/"
            + compe
            + "/submit'>back</a>"
        )
    ...
    engine = create_engine(
        "sqlite:///competitions/" + compe + "/submission.db", echo=False
    )
    session = sessionmaker(bind=engine)()
    # add file contents and upload infomation into database
    add_submitdb(
        user_id=user_name,
        submit_title=submit_title,  # add user_name as id
